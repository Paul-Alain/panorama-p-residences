...
import {
  ...
  CheckCircle2, Pencil, XCircle, ArrowUpDown, ArrowUp, ArrowDown,
  Lock,
} from "lucide-react";
...
const PAGE_SIZE = 20;
const KEYS = ["op-dashboard", "admin-reservations", "op-payments", "admin-occupancy", "op-calendar", "op-revenue", "op-analytics"];
...
/** Badge color per display status */
function statusBadge(s: DisplayResStatus) {
  switch (s) {
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex gap-2">
          <Button variant={view === "active" ? "gold" : "outline"} size="sm"
            onClick={() => setView("active")}>
            Réservations actives
          </Button>
          <Button variant={view === "all" ? "gold" : "outline"} size="sm"
            onClick={() => setView("all")}>
            Historique complet
          </Button>
        </div>
      ...
      <div className="grid gap-2 sm:grid-cols-3">
        <div className="relative sm:col-span-2">
          ...
          <Input value={search} onChange={(e) => setSearch(e.target.value)}
            placeholder="Rechercher (nom, téléphone, e-mail, réf.)" className="pl-9" />
        </div>
        ...
        {DISPLAY_RES_STATUSES.map((s) => {
          ...
          return (
            <button key={s} onClick={() => setStatus(s === status ? "all" : s)}
              className={`rounded-full border px-3 py-1 font-medium transition ${statusBadge(s)} ${status === s ? "ring-2 ring-offset-1" : ""}`}>
                ...
                {pageItems.map((r) => {
                  ...
                  return (
                    <tr key={r.id}
                      ...
                      <td className="px-3 py-2">
                        <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-xs font-medium ${statusBadge(r.displayStatus as DisplayResStatus)}`}>
...
// ── Row actions ────────────────────────────────────────────────────────────
function RowActions({
  r, busyId, locked, onEdit, act, runStatus,
}: {
  r: ResItem;
  busyId: string | null;
  locked: boolean;
  ...
  // Locked = logé or annulée → no actions
  if (locked) {
  ...
  return (
    <>
      {/* Edit button — always visible when not locked */}
      <Button size="sm" variant="ghost" onClick={onEdit} title="Modifier">
        <Pencil className="h-4 w-4" />
      </Button>

      {/* Accept — only for "nouvelle" */}
      ...
      {(r.displayStatus === "nouvelle" || r.displayStatus === "confirmée") && (
        <Button size="sm" variant="ghost"
          ...
          onClick={() => {
            ...
            const ok    = r.displayStatus === "nouvelle"
              ? "Réservation rejetée."
              : "Réservation annulée.";
